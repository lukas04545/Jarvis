...
def api_status():
    ...
    active_pid = runtime.active_provider()
    active_model = llm.get_active_model() if online else None
    model_display = f"{active_pid}:{active_model}" if (active_pid and active_model) else None
    return jsonify(
        {
            "system": "JARVIS",
            "version": __version__,
...
def api_set_settings():
    ...
    valid_keys = {p["runtime_key"] for p in LLM_PROVIDERS.values()} | {"hibp_api_key"}
    runtime.set_keys(**{k: v for k, v in body.items() if k in valid_keys})
...
if __name__ == "__main__":
    active_pid = runtime.active_provider()
    active_name = LLM_PROVIDERS[active_pid]["name"] if active_pid else None
    banner = (
        f"\n  J.A.R.V.I.S. v{__version__}  —  AI core "
        f"{'ONLINE via ' + active_name if active_pid else 'OFFLINE (add a free API key in SETTINGS)'}\n"
