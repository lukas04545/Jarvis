...
| **⌥ DEV Agent (self-coding)** | A DeepSeek-driven coding agent that **reads the codebase, writes changes, runs the test suite, and iterates** — it can keep developing Jarvis itself. It **checkpoints every file it touches and auto-rolls-back to the previous working version if the tests fail** (plus a manual ↺ Rollback). Confined to the repo (no `.git`/secrets) and **disabled unless `JARVIS_ENABLE_DEVAGENT=1`**; a local developer tool. See [Ethics & scope](#ethics--scope). |
